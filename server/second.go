...
func big_step(h *second_handler) {
	h.mu.Lock()
	defer h.mu.Unlock()
...
func (h *second_handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if !h.controller.TryTask() {
		http.Error(w, http.StatusText(http.StatusTooManyRequests), http.StatusTooManyRequests)
		h.log_codes.Println("; 429")
		return
...
func change_step_duration() {
	...
	controller := controllers.CreateCubic(0.3, 0.1)
	...
	second_handler := new(second_handler)
	second_handler.controller = controller
	second_handler.log_codes = logger_codes
	second_handler.log_timings = logger_timings
	http.Handle("/connect_second", second_handler)
